...
def _filtros_relatorios():
    periodos_validos = {"7", "30", "90", "ano", "todos"}
    tipos_validos = {"geral", "materiais", "movimentacoes", "bloqueados"}
    tipos_mov_validos = {"", "Entrada", "Saída", "Transferência", "Correção de localização", "Bloqueio de material"}
    ...
    if tipo_movimentacao not in tipos_mov_validos:
        tipo_movimentacao = ""
    ...
    try:
        ...
        sql_mat = """
            SELECT codigo, descricao, lote, fornecedor, quantidade, unidade,
                   localizacao, status, data_entrada, responsavel, criado_em
            FROM materiais
            WHERE empresa_id = %s
        """
        params_mat = [empresa_id]

        status_material = filtros["status"]
        if filtros["tipo_relatorio"] == "bloqueados":
            status_material = "Bloqueado"

        if status_material:
            sql_mat += " AND status = %s"
            params_mat.append(status_material)

        if filtros["busca"]:
            sql_mat += """
                AND (
                    codigo LIKE %s OR descricao LIKE %s OR lote LIKE %s
                    OR fornecedor LIKE %s OR localizacao LIKE %s OR responsavel LIKE %s
                )
            """
            termo = f"%{filtros['busca']}%"
            params_mat.extend([termo, termo, termo, termo, termo, termo])
        ...
        sql_mov = """
            SELECT m.tipo, m.material_codigo, mat.descricao AS material_descricao,
                   m.origem, m.destino, m.quantidade, m.data, m.responsavel,
                   m.validacao, m.observacao, m.criado_em
            FROM movimentacoes m
            LEFT JOIN materiais mat
              ON mat.empresa_id = m.empresa_id
             AND mat.codigo = m.material_codigo
        ...
        if filtros["busca"]:
            sql_mov += """
                AND (
                    m.material_codigo LIKE %s OR mat.descricao LIKE %s
                    OR m.origem LIKE %s OR m.destino LIKE %s OR m.responsavel LIKE %s
                    OR m.observacao LIKE %s
                )
            """
            termo = f"%{filtros['busca']}%"
            params_mov.extend([termo, termo, termo, termo, termo, termo])
        ...
        total_movimentacoes = len(dados["movimentacoes"])
        total_pendentes = sum(1 for m in dados["materiais"] if m.get("status") == "Pendente")
        total_bloqueados = sum(1 for m in dados["materiais"] if m.get("status") == "Bloqueado")

        cursor.execute("SELECT COUNT(*) AS total FROM etiquetas WHERE empresa_id = %s", (empresa_id,))
        total_etiquetas = cursor.fetchone()["total"]

        entradas = sum(1 for m in dados["movimentacoes"] if m.get("tipo") == "Entrada")
        saidas = sum(1 for m in dados["movimentacoes"] if m.get("tipo") == "Saída")
        transferencias = sum(1 for m in dados["movimentacoes"] if m.get("tipo") == "Transferência")
        bloqueios = sum(1 for m in dados["movimentacoes"] if m.get("tipo") == "Bloqueio de material")
        ...
        for mov in dados["movimentacoes"]:
            ...
            if not data_mov:
                continue
            chave = data_mov.strftime("%Y-%m") if hasattr(data_mov, "strftime") else str(data_mov)[:7]
            if chave in grafico:
                if mov.get("tipo") == "Entrada":
                    grafico[chave]["entrada"] += 1
                elif mov.get("tipo") == "Saída":
                    grafico[chave]["saida"] += 1
                elif mov.get("tipo") == "Transferência":
                    grafico[chave]["transferencia"] += 1
        ...
        dados["tabela_analitica"] = [
            {
                "indicador": "Materiais filtrados",
                "setor": "Estoque geral",
                "resultado": f"{total_materiais} registro(s)",
                "impacto": "Médio",
                "status": "Normal"
            },
            {
                "indicador": "Movimentações filtradas",
                "setor": "Operação",
                "resultado": f"{total_movimentacoes} registro(s)",
                "impacto": "Médio",
                "status": "Normal"
            },
            {
                "indicador": "Materiais pendentes",
                "setor": "Recebimento",
                "resultado": f"{total_pendentes} registro(s)",
                "impacto": "Médio",
            ...
            },
            {
                ...
                "setor": "Qualidade",
                "resultado": f"{total_bloqueados} registro(s)",
        ...
        if total_movimentacoes:
            dados["ocorrencias"].append({"titulo": f"{total_movimentacoes} movimentação(ões)", "descricao": "Registros encontrados conforme os filtros aplicados."})
        if not dados["ocorrencias"]:
            dados["ocorrencias"].append({"titulo": "Nenhum registro crítico", "descricao": "Nenhuma ocorrência relevante encontrada conforme os filtros."})
...
def _linhas_exportacao(dados, tipo_relatorio):
    if tipo_relatorio == "movimentacoes":
        cabecalho = ["Data", "Tipo", "Código", "Material", "Origem", "Destino", "Quantidade", "Responsável", "Validação", "Observação"]
        linhas = [
            [
                str(m.get("data") or ""), m.get("tipo") or "", m.get("material_codigo") or "",
                m.get("material_descricao") or "", m.get("origem") or "", m.get("destino") or "",
                float(m.get("quantidade") or 0), m.get("responsavel") or "", m.get("validacao") or "", m.get("observacao") or ""
...
def _titulo_relatorio(tipo_relatorio):
    if tipo_relatorio == "movimentacoes":
        return "Relatório de Movimentações"
    if tipo_relatorio == "bloqueados":
        return "Relatório de Materiais Bloqueados"
    if tipo_relatorio == "materiais":
        return "Relatório de Materiais"
    return "Relatório Geral de Estoque"
...
def relatorios_pdf():
    ...
    styles.add(ParagraphStyle(name="SmallCell", parent=styles["BodyText"], fontSize=6.5, leading=8))
    story = []

    def add_header():
        story.append(Paragraph("<b>CODESTOCK</b>", styles["Title"]))
        story.append(Paragraph(f"<b>{_titulo_relatorio(filtros['tipo_relatorio'])}</b>", styles["Heading2"]))
        ...
        if filtros.get("status"):
            filtros_txt.append(f"Status: {filtros['status']}")
        if filtros.get("tipo_movimentacao"):
            filtros_txt.append(f"Tipo de movimentação: {filtros['tipo_movimentacao']}")
        if filtros.get("responsavel"):
            filtros_txt.append(f"Responsável: {filtros['responsavel']}")
        if filtros.get("busca"):
            filtros_txt.append(f"Busca: {filtros['busca']}")
        if filtros_txt:
        ...
        resumo_tabela = [
            ...
            [
                resumo.get("total_materiais", 0), resumo.get("total_movimentacoes", 0),
                resumo.get("entradas", 0), resumo.get("saidas", 0),
                resumo.get("transferencias", 0), resumo.get("bloqueios", 0)
            ]
        ]
    ...
    def celula(valor):
        ...
        return Paragraph(texto, styles["SmallCell"])

    def add_table(titulo, cabecalho, linhas):
        ...
        largura_util = landscape(A4)[0] - (16 * mm)
    ...
    if tipo == "geral":
        cab_mat, lin_mat = _linhas_exportacao(dados, "materiais")
        cab_mov, lin_mov = _linhas_exportacao(dados, "movimentacoes")
        add_table("Materiais", cab_mat, lin_mat)
        story.append(PageBreak())
        add_table("Movimentações", cab_mov, lin_mov)
    else:
        cabecalho, linhas = _linhas_exportacao(dados, tipo)
        add_table(_titulo_relatorio(tipo), cabecalho, linhas)
...
def relatorios_excel():
    ...
    def estilizar_planilha(ws, cabecalho, dados_linhas):
        ws.merge_cells(start_row=1, start_column=1, end_row=1, end_column=max(6, len(cabecalho)))
        ws.cell(row=1, column=1, value=f"CODESTOCK - {_titulo_relatorio(filtros['tipo_relatorio'])}")
    ...
    if tipo == "geral":
        ...
        ws.title = "Materiais"
        cab_mat, lin_mat = _linhas_exportacao(dados, "materiais")
        estilizar_planilha(ws, cab_mat, lin_mat)
        ws2 = wb.create_sheet("Movimentações")
        cab_mov, lin_mov = _linhas_exportacao(dados, "movimentacoes")
        estilizar_planilha(ws2, cab_mov, lin_mov)
    else:
        ws = wb.active
        ws.title = "Relatório"
        cabecalho, linhas = _linhas_exportacao(dados, tipo)
        estilizar_planilha(ws, cabecalho, linhas)
